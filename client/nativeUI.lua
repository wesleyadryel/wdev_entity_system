...
function UIMenuItem.New(Text, Description)
    _UIMenuItem = {
        ...
        _Description = Description and tostring(Description) or "",
        SelectedSprite = Sprite.New("commonmenu", "gradient_nav", 0, 0, 431, 38),
        LeftBadge = {
            Sprite = Sprite.New("commonmenu", "", 0, 0, 40, 40),
            Badge = 0
        },
        RightBadge = {
            Sprite = Sprite.New("commonmenu", "", 0, 0, 40, 40),
...
function UIMenuListItem:ItemToIndex(Item)
    for i = 1, #self.Items do
        if type(Item) == type(self.Items[i]) and Item == self.Items[i] then
            return i
        elseif type(self.Items[i]) == "table" and
...
function UIMenuListItem:Draw()
    ...
    local Text = (type(self.Items[self._Index]) == "table") and tostring(self.Items[self._Index].Name) or
                     tostring(self.Items[self._Index] or 'N/A')
...
function UIMenuSliderItem:Position(Y)
    if tonumber(Y) then
        ...
        self.LeftArrow:Position(235 + self.Base._Offset.X + self.Base.ParentMenu.WidthOffset,
...
function UIMenuProgressItem.New(Text, Items, Index, Description, Counter)
    ...
    if _UIMenuProgressItem.Data.Counter then
        ...
        _UIMenuProgressItem.Base:RightLabel((type(_UIMenuProgressItem.Data.Items[_UIMenuProgressItem.Data.Index]) ==
                                                "table") and
                                                tostring(
                _UIMenuProgressItem.Data.Items[_UIMenuProgressItem.Data.Index].Name) or
...
function UIMenuProgressItem:Position(Y)
    if tonumber(Y) then
        self.Base:Position(Y)
        self.Background:Position(8 + self.Base._Offset.X + self.Base.ParentMenu.WidthOffset,
            177 + Y + self.Base._Offset.Y)
        self.Bar:Position(11.75 + self.Base._Offset.X + self.Base.ParentMenu.WidthOffset,
...
function UIMenuProgressItem:CalculateProgress(CursorX)
    ...
    self:Index(math.round(#self.Data.Items *
                              (((Progress >= 0 and Progress <= self.Data.Max) and Progress or
                                  ((Progress < 0) and 0 or self.Data.Max)) / self.Data.Max)))
end
...
function UIMenuHeritageWindow.New(Mum, Dad)
    ...
    _UIMenuHeritageWindow = {
        ...
        MumSprite = Sprite.New("char_creator_portraits", ((Mum < 21) and "female_" .. Mum or "special_female_" ..
            (tonumber(string.sub(Mum, 2, 2)) - 1)), 0, 0, 228, 228),
        DadSprite = Sprite.New("char_creator_portraits", ((Dad < 21) and "male_" .. Dad or "special_male_" ..
            (tonumber(string.sub(Dad, 2, 2)) - 1)), 0, 0, 228, 228),
        Mum = Mum,
        Dad = Dad,
        _Offset = {
            X = 0,
...
function UIMenuHeritageWindow:Index(Mum, Dad)
    ...
    self.MumSprite.TxtName = ((self.Mum < 21) and "female_" .. self.Mum or "special_female_" ..
                                 (tonumber(string.sub(Mum, 2, 2)) - 1))
    self.DadSprite.TxtName = ((self.Dad < 21) and "male_" .. self.Dad or "special_male_" ..
                                 (tonumber(string.sub(Dad, 2, 2)) - 1))
end
...
function UIMenuGridPanel:UpdateParent(X, Y)
    ...
    if ParentType == "UIMenuListItem" then
        ...
        if PanelItemIndex then
            self.ParentItem.Items[PanelItemIndex].Value[self.ParentItem:FindPanelIndex(self)] = {
                X = X,
                Y = Y
            }
            ...
            for Index = 1, #self.ParentItem.Items do
                if type(self.ParentItem.Items[Index]) == "table" then
                    ...
                    self.ParentItem.Items[Index].Panels[PanelIndex] = {
                        X = X,
                        Y = Y
                    }
                else
    ...
    elseif ParentType == "UIMenuItem" then
        self.ParentItem.ActivatedPanel(self.ParentItem.ParentMenu, self.ParentItem, self, {
            X = X,
            Y = Y
        })
    end
...
function UIMenuGridPanel:Functions()
    ...
    if IsMouseInBounds(self.Grid.X + 20 + SafeZone.X, self.Grid.Y + 20 + SafeZone.Y, self.Grid.Width - 40,
        self.Grid.Height - 40) then
            ...
            if not self.Pressed then
                ...
                Citizen.CreateThread(function()
                    ...
                    while IsDisabledControlPressed(0, 24) and
                        IsMouseInBounds(self.Grid.X + 20 + SafeZone.X, self.Grid.Y + 20 + SafeZone.Y,
                            self.Grid.Width - 40, self.Grid.Height - 40) do
                        Citizen.Wait(0)
                        local CursorX, CursorY = math.round(GetControlNormal(0, 239) * 1920) - SafeZone.X -
                                                     (self.Circle.Width / 2),
                            math.round(GetControlNormal(0, 240) * 1080) - SafeZone.Y - (self.Circle.Height / 2)

                        self.Circle:Position(((CursorX > (self.Grid.X + 10 + self.Grid.Width - 40)) and
                                                 (self.Grid.X + 10 + self.Grid.Width - 40) or
                                                 ((CursorX < (self.Grid.X + 20 - (self.Circle.Width / 2))) and
                                                     (self.Grid.X + 20 - (self.Circle.Width / 2)) or CursorX)),
                            ((CursorY > (self.Grid.Y + 10 + self.Grid.Height - 40)) and
                                ...
                                ((CursorY < (self.Grid.Y + 20 - (self.Circle.Height / 2))) and
                                    (self.Grid.Y + 20 - (self.Circle.Height / 2)) or CursorY)))
                    end
                ...
                Citizen.CreateThread(function()
                    while IsDisabledControlPressed(0, 24) and
                        IsMouseInBounds(self.Grid.X + 20 + SafeZone.X, self.Grid.Y + 20 + SafeZone.Y,
                            self.Grid.Width - 40, self.Grid.Height - 40) do
                        Citizen.Wait(75)
                        local ResultX, ResultY = math.round((self.Circle.X - (self.Grid.X + 20) +
                                                                (self.Circle.Width / 2)) / (self.Grid.Width - 40), 2),
                            math.round(
                                (self.Circle.Y - (self.Grid.Y + 20) + (self.Circle.Height / 2)) /
                                    (self.Grid.Height - 40), 2)
...
function UIMenuColourPanel.New(Title, Colours)
    _UIMenuColourPanel = {
        ...
        Text = UIResText.New(Title .. " (1 of " .. #Colours .. ")" or "Title" .. " (1 of " .. #Colours .. ")", 0, 0,
            0.35, 255, 255, 255, 255, 0, "Centre"),
        ParentItem = nil
    }
...
function UIMenuColourPanel:Position(Y) -- required
    if tonumber(Y) then
        ...
        self.SelectedRectangle:Position(15 + (44.5 * ((self:CurrentSelection() - self.Data.Pagination.Min) - 1)) +
                                            ParentOffsetX + (ParentOffsetWidth / 2), 47 + Y)
...
function UIMenuColourPanel:UpdateSelection(PreventUpdate)
    ...
    self.SelectedRectangle:Position(15 + (44.5 * ((CurrentSelection - self.Data.Pagination.Min) - 1)) +
                                        self.ParentItem:Offset().X, self.SelectedRectangle.Y)
    for Index = 1, 9 do
...
function UIMenuColourPanel:Functions()
    ...
    if IsMouseInBounds(self.LeftArrow.X + SafeZone.X, self.LeftArrow.Y + SafeZone.Y, self.LeftArrow.Width,
        self.LeftArrow.Height) then
        if IsDisabledControlJustPressed(0, 24) then
    ...
    if IsMouseInBounds(self.RightArrow.X + SafeZone.X, self.RightArrow.Y + SafeZone.Y, self.RightArrow.Width,
        self.RightArrow.Height) then
        if IsDisabledControlJustPressed(0, 24) then
            if #self.Data.Items > self.Data.Pagination.Total + 1 then
                if self:CurrentSelection() >= self.Data.Pagination.Max then
    ...
    for Index = 1, #self.Bar do
        if IsMouseInBounds(self.Bar[Index].X + SafeZone.X, self.Bar[Index].Y + SafeZone.Y, self.Bar[Index].Width,
            self.Bar[Index].Height) then
            if IsDisabledControlJustPressed(0, 24) then
...
function UIMenuPercentagePanel:Percentage(Value)
    if tonumber(Value) then
        ...
        return math.round(((Progress >= 0 and Progress <= 413) and Progress or ((Progress < 0) and 0 or 413)) /
...
function UIMenuPercentagePanel:Functions()
    ...
    if IsMouseInBounds(self.BackgroundBar.X + SafeZone.X, self.BackgroundBar.Y - 4 + SafeZone.Y,
        self.BackgroundBar.Width, self.BackgroundBar.Height + 8) then
        if IsDisabledControlJustPressed(0, 24) then
            if not self.Pressed then
                self.Pressed = true
                Citizen.CreateThread(function()
                    ...
                    PlaySoundFrontend(self.Audio.Id, self.Audio.Slider, self.Audio.Library, 1)
                    while IsDisabledControlPressed(0, 24) and
                        IsMouseInBounds(self.BackgroundBar.X + SafeZone.X, self.BackgroundBar.Y - 4 + SafeZone.Y,
                            self.BackgroundBar.Width, self.BackgroundBar.Height + 8) do
                        Citizen.Wait(0)
                        local Progress = (math.round(GetControlNormal(0, 239) * 1920) - SafeZone.X) - self.ActiveBar.X
                        self.ActiveBar:Size(((Progress >= 0 and Progress <= 413) and Progress or
                                                ((Progress < 0) and 0 or 413)), self.ActiveBar.Height)
                    end
                    StopSound(self.Audio.Id)
                    ReleaseSoundId(self.Audio.Id)
                    self.Pressed = false
                ...
                Citizen.CreateThread(function()
                    while IsDisabledControlPressed(0, 24) and
                        ...
                        Citizen.Wait(75)
                        local Progress = (math.round(GetControlNormal(0, 239) * 1920) - SafeZone.X) - self.ActiveBar.X
                        self:UpdateParent(math.round(((Progress >= 0 and Progress <= 413) and Progress or
                                                         ((Progress < 0) and 0 or 413)) / self.BackgroundBar.Width, 2))
                    end
                end)
            end
        end
...
function UIMenu.New(Title, Subtitle, X, Y, TxtDictionary, TxtName)
    ...
    if Subtitle ~= "" and Subtitle ~= nil then
        _UIMenu.Subtitle.Rectangle = UIResRectangle.New(0 + _UIMenu.Position.X, 107 + _UIMenu.Position.Y, 431, 37, 0, 0,
            0, 255)
        _UIMenu.Subtitle.Text = UIResText.New(Subtitle, 8 + _UIMenu.Position.X, 110 + _UIMenu.Position.Y, 0.35, 245,
            245, 245, 255, 0)
    ...
    _UIMenu.ArrowSprite = Sprite.New("commonmenu", "shop_arrows_upanddown", 190 + _UIMenu.Position.X, 147 + 37 *
        (_UIMenu.Pagination.Total + 1) + _UIMenu.Position.Y - 37 + _UIMenu.Subtitle.ExtraY, 50, 50)
    _UIMenu.Extra.Up = UIResRectangle.New(0 + _UIMenu.Position.X, 144 + 38 * (_UIMenu.Pagination.Total + 1) +
        _UIMenu.Position.Y - 37 + _UIMenu.Subtitle.ExtraY, 431, 18, 0, 0, 0, 200)
    _UIMenu.Extra.Down = UIResRectangle.New(0 + _UIMenu.Position.X, 144 + 18 + 38 * (_UIMenu.Pagination.Total + 1) +
        _UIMenu.Position.Y - 37 + _UIMenu.Subtitle.ExtraY, 431, 18, 0, 0, 0, 200)
...
-- CONTROLS 
function UIMenu:ProcessControlUiMenu(k)
    ...
    if self.Controls.Back.Enabled and
        (IsDisabledControlJustReleased(0, 177) or IsDisabledControlJustReleased(1, 177) or
            IsDisabledControlJustReleased(2, 177) or IsDisabledControlJustReleased(0, 199) or
            IsDisabledControlJustReleased(1, 199) or IsDisabledControlJustReleased(2, 199)) then
        self:GoBack()
    end
    ...
    if not self.UpPressed then
        if self.Controls.Up.Enabled and
            (IsDisabledControlJustPressed(0, 172) or IsDisabledControlJustPressed(1, 172) or
                IsDisabledControlJustPressed(2, 172) or IsDisabledControlJustPressed(0, 241) or
                IsDisabledControlJustPressed(1, 241) or IsDisabledControlJustPressed(2, 241) or
            ...
            Citizen.CreateThread(function()
                ...
                while self.Controls.Up.Enabled and
                    (IsDisabledControlPressed(0, 172) or IsDisabledControlPressed(1, 172) or
                        IsDisabledControlPressed(2, 172) or IsDisabledControlPressed(0, 241) or
                        IsDisabledControlPressed(1, 241) or IsDisabledControlPressed(2, 241) or
    ...
    if not self.DownPressed then
        if self.Controls.Down.Enabled and
            (IsDisabledControlJustPressed(0, 173) or IsDisabledControlJustPressed(1, 173) or
                IsDisabledControlJustPressed(2, 173) or IsDisabledControlJustPressed(0, 242) or
                IsDisabledControlJustPressed(1, 242) or IsDisabledControlJustPressed(2, 242)) then
            ...
            Citizen.CreateThread(function()
                ...
                while self.Controls.Down.Enabled and
                    (IsDisabledControlPressed(0, 173) or IsDisabledControlPressed(1, 173) or
                        IsDisabledControlPressed(2, 173) or IsDisabledControlPressed(0, 242) or
                        IsDisabledControlPressed(1, 242) or IsDisabledControlPressed(2, 242)) do
                    if #self.Items > self.Pagination.Total + 1 then
                        self:GoDownOverflow()
                    else
                        self:GoDown()
    ...
    if not self.LeftPressed then
        if self.Controls.Left.Enabled and
            (IsDisabledControlPressed(0, 174) or IsDisabledControlPressed(1, 174) or IsDisabledControlPressed(2, 174)) then
    ...
    if not self.RightPressed then
        if self.Controls.Right.Enabled and
            (IsDisabledControlPressed(0, 175) or IsDisabledControlPressed(1, 175) or IsDisabledControlPressed(2, 175)) then
    ...
    if self.Controls.Select.Enabled and
        (IsDisabledControlJustPressed(0, 201) or IsDisabledControlJustPressed(1, 201) or
            IsDisabledControlJustPressed(2, 201)) then
        self:SelectItem()
    end
end
...
function UIMenu:ProcessMouseUIMenu()
    if not self._Visible or self.JustOpened or #self.Items == 0 or tobool(Controller()) or
        not self.Settings.MouseControlsEnabled then
                        ...
                        if Item:Selected() and Item:Enabled() then
                            if SubType == "UIMenuListItem" then
                                ...
                                elseif not IsMouseInBounds(Item.RightArrow.X + SafeZone.X,
                                    Item.RightArrow.Y + SafeZone.Y, Item.RightArrow.Width, Item.RightArrow.Height) then
                                    self:SelectItem()
                                end
...
function UIMenu:RemoveEnabledControl(Inputgroup, Control, Controller)
    ...
    for Index = 1, #self.Settings.EnabledControls[Type] do
        if Inputgroup == self.Settings.EnabledControls[Type][Index][1] and Control ==
            self.Settings.EnabledControls[Type][Index][2] then
...
function MenuPool:AddSubMenu(Menu, Text, Description, KeepPosition, KeepBanner, mouseEnabled)
    if Menu() == "UIMenu" then
        local ItemMenu = UIMenuItem.New(tostring(Text), Description or "")
        local newIndexMenu = Menu:AddItem(ItemMenu)
        local SubMenu
        ...
        self:Add(SubMenu)
        Menu:BindMenuToItem(SubMenu, ItemMenu)
        if mouseEnabled ~= nil then
            local state = mouseEnabled and true or false
            SubMenu.Settings.MouseEdgeEnabled = state
            SubMenu.Settings.MouseControlsEnabled = state
            SubMenu.Settings.ControlDisablingEnabled = state
        end
        return SubMenu, newIndexMenu, ItemMenu
    end
end
...
function NativeUI.toggleEnableComponent(Item, state, v)
    state = state and true or false

    local success, menyType, itemType = pcall(Item)
    if not success or not menyType or not itemType then
        return
    end

    if itemType == "UIMenuCheckboxItem" then
        if Item.Base then
            Item.Base._Enabled = state
        end
        return
    end
    if itemType == "UIMenuItem" then
        Item._Enabled = state
        return
    end
end

function NativeUI.checkedItem(Item, state)
    local success, menyType, itemType = pcall(Item)
    if not success or not menyType or not itemType then
        return
    end
...
function MenuPool:goToFirstElementMenu()
    for k, Menu in pairs(self.Menus) do
        if Menu:Visible() then
            if Menu.Items and #Menu.Items > 0 then
                local current = Menu:CurrentSelection()
                if current then
                    Menu.Items[current]:Selected(false)
                end
                Menu:RefreshIndex()
                if not self.ActiveItem then
                    self.ActiveItem = 1000
                end
                local currentSelection = Menu:CurrentSelection()
                if currentSelection then
                    Menu.Items[currentSelection]:Selected(true)
                    PlaySoundFrontend(-1, Menu.Settings.Audio.UpDown, Menu.Settings.Audio.Library, true)
                    Menu.OnIndexChange(Menu, currentSelection)
                    Menu.ReDraw = true
                end
            end
        end
